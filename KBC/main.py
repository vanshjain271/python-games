...
class KBCGame:
    ...
    def update_fonts(self):
        ...
        self.question_font = font.Font(
            family="Helvetica", size=question_size, weight="bold"
        )
        ...
        self.timer_font = font.Font(
            family="Helvetica", size=question_size * 2, weight="bold"
        )
    ...
    def redisplay_current_question(self):
        ...
        # Draw timer circle
        self.timer_circle = self.timer_canvas.create_oval(
            5, 5, 55, 55, outline=self.colors["timer_normal"], width=3
        )
    ...
    def create_frames(self):
        ...
        self.question_frame = tk.Frame(
            self.game_frame, bg=self.colors["bg_light"], padx=20, pady=20
        )
    ...
    def prepare_game_questions(self):
        ...
        for value in [6000, 7000]:
            penalty_questions = [
                q
                for q in self.all_questions
                if q.get("value") == value and "penalty" in q
            ]
            if penalty_questions:
                ...
                fallback_question = random.choice(
                    [q for q in self.all_questions if q.get("value") >= 5000]
                )
                fallback_question["value"] = value
    ...
    def show_welcome_screen(self):
        ...
        logo_frame = tk.Frame(
            self.welcome_frame, bg=self.colors["bg_light"], height=200
        )
    ...
    def countdown_to_question(self, seconds):
        ...
        if seconds > 0:
            self.timer_id = self.root.after(
                1000, lambda: self.countdown_to_question(seconds - 1)
            )
        else:
        ...
        if self.timer_value > 0:
            ...
            if self.timer_value <= 5:
                self.timer_canvas.itemconfig(
                    self.timer_circle, outline=self.colors["timer_danger"]
                )
                if self.timer_value <= 5 and self.sound_enabled:
                    self.play_sound("timer")
            elif self.timer_value <= 10:
                self.timer_canvas.itemconfig(
                    self.timer_circle, outline=self.colors["timer_warning"]
                )
            else:
                self.timer_canvas.itemconfig(
                    self.timer_circle, outline=self.colors["timer_normal"]
                )
            ...
            self.show_wrong_answer(
                self.game_questions[self.current_question].get("penalty", 0)
            )
